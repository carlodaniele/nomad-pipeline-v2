...
def fetch_telegram_media():
    if not TELEGRAM_BOT_TOKEN:
        print("[Telegram Ingest] TELEGRAM_BOT_TOKEN is not configured.")
        return

    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/getUpdates"
    response = requests.get(url)
    if not response.ok:
        print(f"[Telegram Ingest] getUpdates error: {response.text}")
        return
